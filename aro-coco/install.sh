...
function display_help() {
    ...
    echo " "
    echo "Example usage:"
    echo "# Create ARO cluster and install the GA operator"
    echo " ./install.sh "
    echo " "
    echo "# Create ARO cluster and install the GA operator with image mirroring"
    echo " ./install.sh -m"
    echo " "
    echo "# Create ARO cluster and install the GA operator with additional cluster-wide image pull secret"
    echo " export PULL_SECRET_JSON='{"brew.registry.redhat.io": {"auth": "abcd1234"}, "registry.redhat.io": {"auth": "abcd1234"}}'"
    echo " ./install.sh -s"
    echo " "
    echo "# Create ARO cluster and install the pre-GA operator with image mirroring and additional cluster-wide image pull secret"
    echo " ./install.sh -m -s -b"
    echo " "
    echo "# Deploy only the GA operator"
    echo " ./install.sh -o"
    echo " "
    echo "# Deploy the pre-GA OSC operator with image mirroring and additional cluster-wide image pull secret"
    echo " export PULL_SECRET_JSON='{"brew.registry.redhat.io": {"auth": "abcd1234"}, "registry.redhat.io": {"auth": "abcd1234"}}'"
    echo " ./install.sh -m -s -b -o"
    echo " "
}
